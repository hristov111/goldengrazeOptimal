...
export function loadTikTokPixel(pixelId: string) {
  if (pixelLoaded || typeof window === "undefined") {
    console.log('🚫 TikTok Pixel loading skipped:', { pixelLoaded, windowUndefined: typeof window === "undefined" });
    return;
  }
  ...
  try {
    ...
    (window as any).ttq.ready(() => {
      ...
      eventQueue.forEach(fn => {
        try {
          ...
          console.error('Error processing queued TikTok event:', error);
        }
      });
      eventQueue = [];
    });
    ...
    // Don't let TikTok errors break the app
  }
